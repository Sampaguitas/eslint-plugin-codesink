    ...
    create(context) {
        return {
            MemberExpression(node) {
                ...
                if (codeSink.isRightOperand && !codeSink.isEscaped()) { //&& !isBlocking(context, node)
...
class CodeSink {
    constructor(node) {
        ...
        this.isEscaped = function() {
            ...
            if (isEscapedExclude(node.parent.parent.parent.parent, denyList)) return true;
            if (isEscapedInclude(node.parent.parent.parent.parent, denyList)) return true;
            return false;
        }
        ...
        function isEscapedExclude(ifStatement, denyList) {
            if (!isUnaryCallExpression(ifStatement.test) || !isArrayExpressionIncludes(ifStatement.test.argument) || !isCallExpressionFirstArgument(ifStatement.test.argument)) return false;
            let escapedArguments = ifStatement.test.argument.callee.object.elements;
            return (!!denyList.every(b => Object.keys(escapedArguments).map(e => escapedArguments[e].value).includes(b) ));
        }

        // checks if all elements from the denylist are not present in the array expression ( fe: ["foo", "bar"].includes(attr) )
        function isEscapedInclude(ifStatement, denyList) {
            if (!isArrayExpressionIncludes(ifStatement.test) || !isCallExpressionFirstArgument(ifStatement.test)) return false;
            let escapedArguments = ifStatement.test.callee.object.elements;
            return (!denyList.some(b => Object.keys(escapedArguments).map(e => escapedArguments[e].value).includes(b) ));
        }
        ...
        function isCallExpressionFirstArgument(callExpression) {
            let isMemberExpression = node.property.type === "MemberExpression";
            let this_attribute = node.property.type !== "MemberExpression" ? node.property.name : "";
            let this_array = node.property.type === "MemberExpression" ? node.property.object.name : "";
            let this_iterator = node.property.type === "MemberExpression" ? node.property.property.name : "";
            ...
            if (isMemberExpression) {
                ...
                if (callExpression.arguments[0].object.name !== this_array) return false;
                if (callExpression.arguments[0].property.type !== "Identifier") return false;
                return callExpression.arguments[0].property.name === this_iterator;
            } else {
                if (callExpression.arguments[0].type !== "Identifier") return false;
                return callExpression.arguments[0].name === this_attribute;
            }
        }
